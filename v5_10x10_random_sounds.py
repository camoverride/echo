...
# Load all audio files in the "audio" folder and randomly assign one to each cell in the 10x10 grid
audio_folder = "audio"
audio_files = [f"{audio_folder}/{file}" for file in os.listdir(audio_folder) if file.endswith(".wav")]
random.shuffle(audio_files)  # Shuffle the list for random assignment

# Ensure there are exactly 100 audio files for a 10x10 grid
if len(audio_files) < 100:
    ...
    audio_files = audio_files[:100]  # Limit to 100 files if there are extras
...
# Create a 10x10 grid of random colors that fills the screen
